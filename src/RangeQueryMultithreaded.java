...
public class RangeQueryMultithreaded {
    ...
    void addInterval(MergeInterval newInterval)
    {
    	...
    	else
    	{
    		...
    		int begin=0;
    		int end=0;
    		/* The first case handles if interval falls outside the range */
    		while(i<textRanges.size() && newInterval.left.compareTo(textRanges.get(i).right)>=0)		
    		{
    			i++;
    			begin=i;
    		}
    		/* This handles when interval intersects with range */
    ...
    boolean deleteInterval(MergeInterval newInterval) throws IntervalException
    {
    	synchronized (textRanges) {
    	if(textRanges.size()==0)
    		throw new IntervalException("Deletion from empty list");
    ...
    public static void main(String[] args) throws IntervalException
    {
    	// Testing Insertion:
    	System.out.println("\nTESTING INSERTION");
    	RangeQueryMultithreaded rq= new RangeQueryMultithreaded();
    	...
    	// Testing Deletion:
    	System.out.println("\nTESTING DELETION");
    	rq= new RangeQueryMultithreaded();
    	rq.addInterval(new MergeInterval("AaA","BaB",true,true));
    	rq.addInterval(new MergeInterval("Aac","CaC",true,true));
    	rq.addInterval(new MergeInterval("Dd","Df",true,true));
    ...
    //  Interval l = new Interval("Aaab","De",true,true);
    	...
    	if(result)
    	   rq.printList();
    		...
    		throw new IntervalException("Range not found in list");
